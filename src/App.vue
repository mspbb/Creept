  ...
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    <div class="container">
      ...
      <template v-if="tickers.length > 0">
        ...
        <div>Фильтр: <input v-model="filter" v-on:input="page = 1" /></div>
...
export default {
  ...
  created() {
    ...
    const tickersData = localStorage.getItem("creept-list");
  ...
  methods: {
    ...
    add(ticker) {
      ...
      if (this.validOfRepeat(currentTicker.name)) {
        this.typeOfValid = this.validOfRepeat(currentTicker.name);
        return;
      }
      if (this.validOfExistence(currentTicker.name)) {
        this.typeOfValid = this.validOfExistence(currentTicker.name);
        return;
      }
    ...
    //мое
    validOfRepeat(checkedThicker) {
      let repeatResult = "";
      this.tickers.find((e) => {
        if (e.name == checkedThicker) {
          return (repeatResult = "Такой тикет уже есть");
        }
      });
      return repeatResult;
    },
    validOfExistence(checkedThicker) {
      let validRefuse = "Такого тикита не существует";
      let arrData = this.creeptArr.Data;
      for (this.key in arrData) {
        if (arrData[this.key].Symbol == checkedThicker) {
          validRefuse = "";
          return;
        }
      }
  ...
  watch: {
    tickers() {
      localStorage.setItem("creept-list", JSON.stringify(this.tickers));
    },
    selectedTicker() {
      this.graph = [];
      this.$nextTick().then(() => {
